...
If Redis is unavailable during relay startup, the process starts in degraded mode, reports unhealthy backplane status, and retries Redis recovery with bounded backoff. When recovery succeeds, active local rooms are resubscribed to the backplane and existing local peers are reconciled into distributed admission and presence without waiting for a new peer join.

...
`MMN_SYNC_RECEIVE_TIMEOUT_SECONDS` bounds how long a joined peer can take to complete one WebSocket message. Keep it above the browser client's heartbeat interval so healthy idle sockets stay open while slow fragmented sends still cannot hold relay slots indefinitely.
